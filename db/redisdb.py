...
class RedisDB:
    def __init__(
        ...
        **kwargs,
        ...
        if ip_ports is None:
            ip_ports = setting.REDIS_DB_IP_PORTS
        if db is None:
            db = setting.REDIS_DB_DB
        if user_pass is None:
            user_pass = setting.REDIS_DB_USER_PASS
        if service_name is None:
            service_name = setting.REDIS_DB_SERVICE_NAME
        if kwargs is None:
            kwargs = setting.REDIS_DB_KWARGS
