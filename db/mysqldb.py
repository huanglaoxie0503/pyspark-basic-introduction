...
from utils import settings as setting
...
class MySQLDB(object):
    ...
    def __init__(
            self, host=None, port=None, db=None, user=None, passwd=None, **kwargs
    ):
        ...
        if not passwd:
            passwd = setting.MYSQL_USER_PASS
        ...
        try:
            ...
            )  # cursor class 使用服务的游标，默认的在多线程下大批量插入数据会使内存递增
    ...
    def size_of_connect_pool(self):
        ...
        @return:
        """
        return len(self.connect_pool.idle_cache)

    @auto_retry
    def query(self, sql, limit=0, to_json=False, convert_col=True):
        """
        @summary:
        无数据： 返回()
        有数据： 若limit == None 则返回 (data1, data2)
                否则返回 ((data1, data2),)
        ---------
        @param sql:
        @param limit:
    ...
    def add_batch(self, sql, rows: List[List]):
        """
        @summary: 批量添加数据
        ---------
        @ param sql: insert ignore into (xxx,xxx,xxx) values (%s, %s, %s)
        @ param rows: 列表 [[v1,v2,v3], [v1,v2,v3]]
                       列表里的值要和插入的key的顺序对应上
        ---------
        @result: 添加行数
        """
        ...
        try:
            conn, cursor = self.get_connection()
            affect_count = cursor.executemany(sql, rows)
            conn.commit()
        except Exception as e:
            log.error(
                """
