...
def memoize_method_noargs(method):
...
def format_time(release_time, date_format="%Y-%m-%d %H:%M:%S"):
    ...
    if "年前" in release_time:
        ...
        n_days = re.compile(r"(\d+)\s*天前").findall(release_time)
        days_ago = datetime.datetime.now() - datetime.timedelta(days=int(n_days[0]))
        release_time = days_ago.strftime("%Y-%m-%d %H:%M:%S")

    elif "小时前" in release_time:
        n_hours = re.compile(r"(\d+)\s*小时前").findall(release_time)
        hours_ago = datetime.datetime.now() - datetime.timedelta(hours=int(n_hours[0]))
        release_time = hours_ago.strftime("%Y-%m-%d %H:%M:%S")

    elif "分钟前" in release_time:
        n_minutes = re.compile(r"(\d+)\s*分钟前").findall(release_time)
        minutes_ago = datetime.datetime.now() - datetime.timedelta(
            minutes=int(n_minutes[0])
        )
...
def get_before_date(
    ...
    @param return_date_format: 返回的时间格式
    @param current_date_format: 时间格式
    ---------
    @result: 字符串
    """
...
def get_uuid(key1="", key2=""):
    ...
    if not key1 and not key2:
        ...
        r_hash = md5(bytes(key1, "utf-8") + bytes(key2, "utf-8")).digest()
        uuid_object = uuid.UUID(bytes=r_hash[:16], version=3)
...
def format_sql_value(value):
    if isinstance(value, str):
        ...
        value = dumps_json(value, indent=4)
...
def reach_freq_limit(rate_limit, *key):
    ...
    try:
        if get_redis_db().strget(key):
            return True

        get_redis_db().strset(key, time.time(), ex=rate_limit)
